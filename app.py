...
with tab_metodo:
    ...
    st.dataframe(dataframe_estilizado(diseno), width="stretch", hide_index=True)

    st.markdown("### Matriz de cumplimiento")
    cumplimiento = pd.DataFrame(
        {
            "Requerimiento del TP": [
                "Ingresar a, b, c y d",
                "Ingresar subsidio y precio maximo",
                "Situacion inicial completa",
                "Situacion posterior completa",
                "Resultado fiscal",
                "Curvas y situaciones en graficos",
                "Excedentes relevantes",
                "Simulaciones obligatorias",
                "Interpretacion economica",
                "Comparar y exportar resultados",
            ],
            "Implementacion en la aplicacion": [
                "Dos paneles independientes y editables; cada cambio recalcula todo el ejercicio.",
                "Controles funcionales e independientes para ambas politicas.",
                "Precio, cantidad, EC, EP y bienestar total.",
                "Nuevos precios/cantidad, nuevos excedentes y variacion del bienestar.",
                "Gasto y resultado fiscal del subsidio; resultado fiscal directo del precio maximo.",
                "Demanda, oferta, equilibrio inicial, intervencion, escasez y perdida de bienestar.",
                "Metricas antes/despues y grafico comparativo de bienestar.",
                "Subsidios 0/5/10/15/20 y precios maximos 70/60/50/40/30.",
                "Ganadores, perdedores, financiamiento, escasez y conclusion dinamica.",
                "Tablas, graficos comparativos y libro Excel consolidado.",
            ],
            "Estado": ["Cumplido"] * 10,
        }
    )
    st.dataframe(dataframe_estilizado(cumplimiento), width="stretch", hide_index=True)

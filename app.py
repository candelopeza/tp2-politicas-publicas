...
st.markdown(
    ...
    <style>
        ...
        .stApp {{
            background:
                radial-gradient(circle at 88% 4%, rgba(15,157,138,.10), transparent 24rem),
                linear-gradient(180deg, #F9FBFE 0%, {PAPER} 100%);
            color: {NAVY};
            font-family: "DM Sans", "Segoe UI", sans-serif;
        }}
        [data-testid="stHeader"] {{ background: rgba(249,251,254,.82); }}
        [data-testid="stSidebar"] {{
            background: linear-gradient(180deg, #0E263D 0%, #123C4A 100%);
        ...
        h1, h2, h3 {{ color: {NAVY}; letter-spacing: -.025em; }}
        h1 {{ font-family: "Libre Baskerville", Georgia, serif; }}
        .block-container {{
            max-width: 1480px;
            padding-top: 1.6rem;
            padding-bottom: 4rem;
        ...
        .hero {{
            ...
            border-radius: 26px;
            background: linear-gradient(125deg, #0D2942 0%, #12485A 62%, #0D746D 100%);
        ...
        [data-testid="stMetricLabel"] {{ color: {SLATE}; font-weight: 600; }}
        ...
        .stButton > button, .stDownloadButton > button {{
            border: 0;
            border-radius: 12px;
            background: linear-gradient(135deg, {NAVY}, #176B72);
            color: white;
            font-weight: 700;
            min-height: 2.8rem;
        ...
        .soft-rule {{ border: 0; border-top: 1px solid #DFE8F0; margin: 1.5rem 0; }}
        .source-note {{
            color: {SLATE};
            font-size: .79rem;
            line-height: 1.5;
...
def moneda(valor: float) -> str:
    ...
    return f"{signo}$ {abs(valor):,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
...
def dataframe_estilizado(df: pd.DataFrame, formatos: dict | None = None):
    ...
    return df.style.format(formatos).set_properties(
        **{"color": NAVY, "font-family": "DM Sans, Segoe UI, sans-serif"}
    )
...
def estilo_figura(fig: go.Figure, titulo: str, subtitulo: str = "") -> go.Figure:
    fig.update_layout(
        ...
        font=dict(family="DM Sans, Segoe UI, sans-serif", color=NAVY),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor=WHITE,
        hoverlabel=dict(bgcolor=WHITE, font_color=NAVY),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        margin=dict(l=25, r=20, t=95, b=25),
        height=530,
    )
    fig.update_xaxes(
        title="Cantidad",
        gridcolor="#E7EDF3",
        zerolinecolor="#CBD7E2",
        rangemode="tozero",
    )
    fig.update_yaxes(
        title="Precio",
...
def grafico_subsidio(a: float, b: float, c: float, d: float, r: dict) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=q, y=pdem, name="Demanda", line=dict(color=BLUE, width=3)))
    fig.add_trace(go.Scatter(x=q, y=pof, name="Oferta inicial", line=dict(color=TEAL, width=3)))
    fig.add_trace(
        go.Scatter(
            x=q,
            y=pof_sub,
    ...
    fig.add_annotation(
        ...
        text="Gasto fiscal",
        showarrow=False,
        font=dict(color="#9A6304", size=12),
    )
    fig.add_annotation(
        x=r["q1"],
        y=r["pu"],
...
def grafico_precio_maximo(a: float, b: float, c: float, d: float, r: dict) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=q, y=pdem, name="Demanda", line=dict(color=BLUE, width=3)))
    fig.add_trace(go.Scatter(x=q, y=pof, name="Oferta", line=dict(color=TEAL, width=3)))
    fig.add_trace(
        go.Scatter(
            x=[r["q0"]],
            y=[r["p0"]],
...
def grafico_bienestar(etiquetas: list[str], inicial: list[float], final: list[float], titulo: str) -> go.Figure:
    ...
    fig.update_yaxes(gridcolor="#E7EDF3")
...
def grafico_simulacion_subsidios(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        yaxis=dict(title="Cantidad", gridcolor="#E7EDF3"),
        yaxis2=dict(title="Gasto publico", overlaying="y", side="right", showgrid=False),
        xaxis=dict(title="Subsidio por viaje", gridcolor="#E7EDF3"),
        height=455,
    )
    return estilo_figura(
        fig,
...
def grafico_simulacion_precios(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=df["Escasez"],
            name="Escasez",
            marker_color="rgba(220,76,100,.34)",
        )
    )
    fig.update_layout(
        xaxis=dict(title="Precio maximo", autorange="reversed", gridcolor="#E7EDF3"),
        yaxis=dict(title="Cantidad", gridcolor="#E7EDF3"),
        height=455,
    )
    return estilo_figura(
...
def parametros_sidebar():
    ...
    st.sidebar.caption("Modifica los parametros y observa como cambia cada resultado.")
    st.sidebar.markdown("---")
    mercado = st.sidebar.radio(
        "Mercado a configurar",
        ["Transporte subsidiado", "Alquileres con precio maximo"],
    )

    if mercado == "Transporte subsidiado":
        st.sidebar.markdown("### Demanda y oferta")
        a = st.sidebar.number_input("a · Intercepto de demanda", min_value=1.0, value=1500.0, step=50.0)
        b = st.sidebar.number_input("b · Pendiente de demanda", min_value=0.01, value=25.0, step=1.0)
        c = st.sidebar.number_input("c · Intercepto de oferta", value=0.0, step=10.0)
        d = st.sidebar.number_input("d · Pendiente de oferta", min_value=0.01, value=15.0, step=1.0)
        st.sidebar.markdown("### Politica publica")
        politica = st.sidebar.number_input("Subsidio por viaje", min_value=0.0, value=8.0, step=1.0)
        valores = {"a": a, "b": b, "c": c, "d": d, "politica": politica}
    else:
        st.sidebar.markdown("### Demanda y oferta")
        a = st.sidebar.number_input("a · Intercepto de demanda", min_value=1.0, value=1800.0, step=50.0)
        b = st.sidebar.number_input("b · Pendiente de demanda", min_value=0.01, value=20.0, step=1.0)
        c = st.sidebar.number_input("c · Intercepto de oferta", value=0.0, step=10.0)
        d = st.sidebar.number_input("d · Pendiente de oferta", min_value=0.01, value=12.0, step=1.0)
        st.sidebar.markdown("### Politica publica")
        politica = st.sidebar.number_input("Precio maximo", min_value=0.0, value=40.0, step=1.0)
        valores = {"a": a, "b": b, "c": c, "d": d, "politica": politica}
    ...
    st.sidebar.caption("Los valores iniciales corresponden exactamente a las consignas del TP.")
    return mercado, valores
...
mercado_activo, v = parametros_sidebar()
...
tab_inicio, tab_subsidio, tab_precio, tab_simulacion, tab_metodo = st.tabs(
    [
        "Panorama",
        "01 · Subsidio",
        "02 · Precio maximo",
...
# Datos del mercado activo para reutilizar entre solapas.
if mercado_activo == "Transporte subsidiado":
    sub_params = v
    pm_params = {"a": 1800.0, "b": 20.0, "c": 0.0, "d": 12.0, "politica": 40.0}
else:
    sub_params = {"a": 1500.0, "b": 25.0, "c": 0.0, "d": 15.0, "politica": 8.0}
    pm_params = v
...
with tab_subsidio:
    ...
    if not sub_ok:
        ...
        st.markdown("### Resultado central")
        k1, k2, k3, k4, k5 = st.columns(5)
        k1.metric("Precio inicial", moneda(sub_r["p0"]))
        k2.metric("Precio usuarios", moneda(sub_r["pu"]), delta=delta_moneda(sub_r["pu"] - sub_r["p0"]))
        k3.metric("Precio empresas", moneda(sub_r["po"]), delta=delta_moneda(sub_r["po"] - sub_r["p0"]))
        k4.metric("Cantidad final", numero(sub_r["q1"]), delta=f"{sub_r['delta_q']:+.2f}")
        k5.metric("Gasto publico", moneda(sub_r["gasto"]))
        ...
        with st.expander("Ver tabla completa del ejercicio"):
            st.dataframe(
                dataframe_estilizado(resumen_sub, {"Valor": "{:,.2f}"}),
                use_container_width=True,
                hide_index=True,
            )
...
with tab_precio:
    ...
    if not pm_ok:
        ...
        st.markdown("### Resultado central")
        k1, k2, k3, k4, k5 = st.columns(5)
        k1.metric("Precio de equilibrio", moneda(pm_r["p0"]))
        k2.metric("Cantidad de equilibrio", numero(pm_r["q0"]))
        ...
        with st.expander("Ver tabla completa del ejercicio"):
            st.dataframe(
                dataframe_estilizado(resumen_pm, {"Valor": "{:,.2f}"}),
                use_container_width=True,
                hide_index=True,
            )
...
with tab_simulacion:
    ...
    st.markdown("### Subsidios: $0, $5, $10, $15 y $20")
    if sub_ok:
        st.dataframe(
            dataframe_estilizado(
                df_sim_sub,
                {
                    ...
                    "Precio usuarios": "${:,.2f}",
                    "Precio empresas": "${:,.2f}",
                    "Cantidad de equilibrio": "{:,.2f}",
                    "Gasto publico": "${:,.2f}",
                    "Bienestar total": "${:,.2f}",
                    "Variacion de bienestar": "${:,.2f}",
                },
            ),
            use_container_width=True,
            hide_index=True,
        )
        st.plotly_chart(grafico_simulacion_subsidios(df_sim_sub), use_container_width=True, config={"displayModeBar": False})
    ...
    st.markdown("<hr class='soft-rule'>", unsafe_allow_html=True)
    st.markdown("### Precios maximos: $70, $60, $50, $40 y $30")
    if pm_ok:
        st.dataframe(
            dataframe_estilizado(
                df_sim_pm,
                {
                    "Precio maximo": "${:,.2f}",
                    "Cantidad demandada al techo": "{:,.2f}",
                    "Cantidad ofrecida al techo": "{:,.2f}",
                    "Escasez": "{:,.2f}",
                    "Cantidad transada": "{:,.2f}",
                    "Bienestar total": "${:,.2f}",
                    "Variacion de bienestar": "${:,.2f}",
                },
            ),
        ...
        st.plotly_chart(grafico_simulacion_precios(df_sim_pm), use_container_width=True, config={"displayModeBar": False})
...
# =============================================================================
# METODOLOGIA Y DOCUMENTACION TECNICA
# =============================================================================

with tab_metodo:
    titulo_seccion(
        "Documentacion tecnica",
        "Diseño del sistema y supuestos",
        "Sintesis preparada para cubrir la documentacion breve solicitada en el trabajo practico.",
    )
    ...
    with t1:
        tarjeta(
            ...
            "Modelar mercados lineales y evaluar automaticamente como un subsidio o un precio maximo "
            "afectan precios, cantidades, excedentes, resultado fiscal y bienestar social.",
        )
    with t2:
        tarjeta(
            "Funcionalidades",
            "Que implementa",
            "Parametros editables, calculos automaticos, graficos interactivos, comparacion de escenarios, "
            "interpretaciones dinamicas y exportacion de resultados a Excel.",
        )

    st.markdown("### Diseño del sistema")
    diseno = pd.DataFrame(
        {
            "Componente": [
                "Entradas de mercado",
                "Entradas de politica",
                "Salidas iniciales",
                "Salidas posteriores",
            ...
            "Detalle": [
                "a y b de demanda; c y d de oferta.",
                "Monto del subsidio o nivel del precio maximo.",
                "Precio, cantidad, EC, EP y bienestar total.",
                "Nuevos precios/cantidad, excedentes, gasto fiscal o escasez y variacion de bienestar.",
                "equilibrio, excedente_consumidor, excedente_productor, resultado_subsidio y resultado_precio_maximo.",
                "Plotly: curvas, equilibrios, brecha del subsidio, escasez y perdida de bienestar.",
                "Libro Excel con resumenes y simulaciones obligatorias.",
            ],
        }
    )
    st.dataframe(dataframe_estilizado(diseno), use_container_width=True, hide_index=True)
    ...
    with f2:
        tarjeta(
            "Subsidio unitario s",
            "Po = Pu + s",
            "Pu = (a - c - ds) / (b + d)<br>Gasto fiscal = s · Q",
            "formula-card",
        )
    with f3:
        tarjeta(
            "Precio maximo",
            "Escasez = Qd - Qo",
            "Si Pmax &lt; P*, la cantidad transada queda limitada por la oferta.",
            "formula-card",
        )

    st.markdown("### Supuestos y alcance")
    st.markdown(
        """
        - La economia es cerrada y no existe comercio exterior.
        - Oferta y demanda son lineales; los agentes son tomadores de precios.
        - No se incorporan externalidades, costos administrativos ni efectos distributivos del financiamiento.
        - El gasto publico se resta del bienestar social.
        - Bajo precio maximo se supone asignacion eficiente: las unidades disponibles llegan a los consumidores
          con mayor disposicion a pagar. Si la asignacion fuera aleatoria, el excedente del consumidor podria ser menor.
        - Un precio maximo por encima del equilibrio no es vinculante y, por lo tanto, no cambia el resultado del mercado.
        """
    )

    st.info(
        "Para las capturas requeridas en el informe, las pantallas mas representativas son: "
        "resultado central de cada ejercicio, grafico de mercado y tablas de simulacion."
    )
